...
query_redd_data <- function(
  ...
  if (stringr::str_detect(redd_file_name, "Wenatchee") |
    ...
    # get experience data
    exp_df <- readxl::read_excel(
      paste(experience_path,
    ...
    # get experience data
    exp_df <- suppressMessages(readxl::read_excel(
      paste(experience_path,
        experience_file_name,
        sep = "/"
      ),
  ...
  redd_df <- redd_surv_df |>
    ...
    dplyr::mutate(
      naive_density_km = visible_redds / length_km,
      exp_sp_total_log = log(exp_sp_total)
    )

  if (nrow(redd_df) == 0) {
    message(paste(
      "No redd data found for",
