...
prep_uc_sthd_data <- function(
  ...
  if (is.null(redd_file_name)) {
    redd_file_name <- dplyr::case_when(
      basin == "Wenatchee" ~ "Wenatchee_Redd_Surveys.xlsx",
  ...
  fpr_all <- basin_tags_all |>
    ...
    dplyr::summarize(
      n_male = dplyr::n_distinct(tag_code[sex == "M"]),
